...
# Add these (keep your existing HOST/PORT if you want local fallback)
MILVUS_URI = os.getenv("MILVUS_URI")  # e.g. xxxxxxxx.api.gcp-us-west1.zillizcloud.com:19530
...
def get_vectorstore():
    """
    Prefer Zilliz Cloud (uri + token + secure). Falls back to host/port if URI not set.
    """
    collection = os.getenv("MILVUS_COLLECTION", COLLECTION_NAME)

    if MILVUS_URI:
        # Zilliz Cloud (TLS). Ensure https:// is present when secure=True
        uri = MILVUS_URI if MILVUS_URI.startswith(("http://", "https://")) else f"https://{MILVUS_URI}"
        conn_args = {"uri": uri, "token": MILVUS_TOKEN, "secure": MILVUS_SECURE}
    else:
        # Local/VM/GKE fallback
    ...
    try:
        vs = Milvus(
            ...
            # Optional: nudge the index to something sensible for normalized embeddings
...
# Initialize with new function (no args)
database: Optional[Milvus] = get_vectorstore()
